...
repeats = 3
optimizer_name = 'SGD'
directory_name = str(dataset) + '_PredictiveVariance_' + str(dropout_prob) + 'dropout_'+ str(num_training) + 'Train' + '_StepLR'
    ...
    with torch.no_grad():
        ...
        # for iter, (input, target, label) in enumerate(val_dataloader):
            ...
            for out in outs:
                N, _, h, w = out.shape
                # out_ is y_predict...
                out_ = out[:, :num_classes,: ,:].detach().clone()
            ...
            epistemic_uncertainty = np.mean(np.stack(outs_sm), axis = 0) - (np.mean(np.stack(outs_sm), axis = 0))**2
...
epistemic_uncertainty_averaged = np.nanmean(np.stack(epistemic_uncertainty_full), axis = 0)
...
for r in range(repeats):
    ...
    with torch.no_grad():
        ...
        # for iter, (input, target, label) in enumerate(val_dataloader):
            ...
            N, _, h, w = out.shape
            out_ = out[:,:num_classes,:,:].detach().clone()
            var = out[:,num_classes:,:,:].detach().clone().cpu().numpy()
            label_ = label.detach().clone()
            out__ = softmax(out_)
            out__ = out__.cpu().numpy()
            accuracy = utils.pixel_accuracy(out_,label_) # batch accuracy
            iou = utils.iou(out_, label_, per_image=True)
            samples_acc.append(accuracy)
            samples_IU.append(iou)
            ...
            aleatoric_uncertainty = np.exp(var)
            aleatoric_uncertainty_repeat.append(aleatoric_uncertainty) # name change needed .. batch_aleatoric_uncertainty
            batch_accuracy.append(np.nanmean(np.asarray(samples_acc), axis = 0))
            batchIU.append(np.nanmean(np.asarray(samples_IU), axis = 0))
            batchF1.append(np.nanmean(np.asarray(samples_F1), axis = 0))
    aleatoric_F1_full.append(np.hstack(batchF1))
    aleatoric_uncertainty_full.append(np.vstack(aleatoric_uncertainty_repeat))
aleatoric_uncertainty_averaged = np.mean(np.stack(aleatoric_uncertainty_full), axis = 0) # Average across the repeats 
...
with open(str(dataset) + '_' + str(dropout_prob) + 'dropout_' + '80mcsamples_PredictiveVariance_RoadCrackTest_uq_122521.txt' , 'a') as file:
